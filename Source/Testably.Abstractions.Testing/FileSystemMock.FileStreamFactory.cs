...
public sealed partial class FileSystemMock
{
    private sealed class FileStreamFactoryMock : IFileSystem.IFileStreamFactory
    {
        ...
        public FileSystemStream New(string path, FileMode mode)
            => New(path, mode, mode == FileMode.Append ? FileAccess.Write : FileAccess.ReadWrite, DefaultShare, DefaultBufferSize, DefaultUseAsync);
        ...
        public FileSystemStream New(string path,
                                    ...
                                    int bufferSize,
                                    FileOptions options)
            => new FileStreamMock(_fileSystem, path, mode, access, share, bufferSize, options);
        ...
        public FileSystemStream New(string path, FileStreamOptions options)
            => New(path, options.Mode, options.Access, options.Share, options.BufferSize, options.Options);
#endif

        /// <inheritdoc cref="IFileSystem.IFileStreamFactory.Wrap(FileStream)" />
        public FileSystemStream Wrap(FileStream fileStream)
            => throw new NotSupportedException("You cannot wrap an existing FileStream in the FileSystemMock instance!");

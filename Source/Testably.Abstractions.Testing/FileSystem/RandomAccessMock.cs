...
internal sealed class RandomAccessMock : IRandomAccess
{
	...
	public int Read(SafeFileHandle handle, Span<byte> buffer, long fileOffset)
	{
		...
		byte[] bytes = ReadBytes(handle, fileOffset);
		int count = Math.Min(buffer.Length, bytes.Length);
	...
	private static long Scatter(byte[] bytes, IReadOnlyList<Memory<byte>> buffers)
	{
		int position = 0;
	...
	/// <summary>
	///     Returns the bytes of the file that <paramref name="handle" /> refers to, starting at
	///     <paramref name="fileOffset" />; an offset at or beyond the end yields an empty result, which is the short
	///     read that <see cref="RandomAccess" /> reports as zero bytes.
	/// </summary>
	private byte[] ReadBytes(SafeFileHandle handle, long fileOffset)
	{
		if (fileOffset < 0)
		{
		...
		byte[] bytes = GetContainer(handle, FileAccess.Read).GetBytes();
		if (fileOffset >= bytes.Length)
		{
			return [];
		}

		byte[] result = new byte[bytes.Length - fileOffset];
		Array.Copy(bytes, fileOffset, result, 0, result.Length);
		return result;
	}

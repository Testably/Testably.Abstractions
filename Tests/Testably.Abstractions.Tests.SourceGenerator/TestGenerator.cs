...
public class TestGenerator : ISourceGenerator
{
	...
	public void Execute(GeneratorExecutionContext context)
	{
		...
		foreach (ClassToGenerate classToGenerate in _receiver.ClassesToGenerate)
		{
			...
			CreateRealFileSystemClass(context, classToGenerate);
		}
	}
...
{{
	...
	public void Dispose()
		...
		string? fileName = CreateClassNamePrefix(classToGenerate) + ".MockFileSystem.cs";
		context.AddSource(fileName,
			SourceText.From(sourceBuilder.ToString(), Encoding.UTF8));
	}

	private static void CreateRealFileSystemClass(GeneratorExecutionContext context,
	                                              ClassToGenerate classToGenerate)
	{
		StringBuilder? sourceBuilder = new(@$"#if !DEBUG || !DISABLE_TESTS_REALFILESYSTEM
...
{{
	...
	public void Dispose()
		...
		string? fileName = CreateClassNamePrefix(classToGenerate) + ".RealFileSystem.cs";
		context.AddSource(fileName,
			SourceText.From(sourceBuilder.ToString(), Encoding.UTF8));
	}

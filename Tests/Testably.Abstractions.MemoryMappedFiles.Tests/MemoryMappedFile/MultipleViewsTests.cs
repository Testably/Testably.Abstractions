	...
	: FileSystemTestBase(testData)
{
	...
	public async Task ViewStreamAndAccessor_ShouldShareData()
	{
		...
		using (System.IO.Stream stream = mappedFile.CreateViewStream(0, 50))
		{
			stream.Write([1, 2, 3, 4,], 0, 4);
			stream.Flush();
		}

		await That(accessor.ReadByte(0)).IsEqualTo((byte)1);
		await That(accessor.ReadByte(3)).IsEqualTo((byte)4);
	}
}

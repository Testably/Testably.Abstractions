...
On the mock, `Frequency` is fixed at `TimeSpan.TicksPerSecond * 10` = 100,000,000 (100 MHz, i.e. 10 ns per timestamp tick) and `IsHighResolution` is always `true` - matching what every modern OS reports for the real `Stopwatch`.

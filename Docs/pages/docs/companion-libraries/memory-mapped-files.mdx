...
`MemoryMappedFileSystemViewStream` derives from `UnmanagedMemoryStream` - the same base class as the real `MemoryMappedViewStream` - so it inherits members like `Capacity` directly and only adds the `PointerOffset` member specific to `MemoryMappedViewStream`.

...
Two details tied to operating-system memory mapping are intentionally simplified: a view created without an explicit size has a `Capacity` of exactly the remaining bytes (the real file system rounds up to the system page size), and `PointerOffset` is always `0`. Exception messages of the mock follow the Windows behaviour of the BCL, regardless of the simulated operating system of the `MockFileSystem`.
